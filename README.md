...
plugins:
  enabled: true
  store-sources:
    - https://raw.githubusercontent.com/shelken/cpa-codex-autoban/main/registry.json
```
...
```powershell
cd cpa-codex-autoban
.\build.ps1            # Windows
# 或
bash build.sh          # 任意平台
```

成功后会生成 `cpa-codex-autoban.dll`（Windows）。
...
把 dll 放进去即可（推荐 `plugins/windows/amd64/cpa-codex-autoban.dll`）。

**插件 ID = 文件名去掉扩展名**，即 `cpa-codex-autoban`。
...
```text
/v0/resource/plugins/cpa-codex-autoban/status
```
...
curl -H "Authorization: Bearer $CPA_MANAGEMENT_KEY" \
  http://localhost:8317/v0/management/plugins/cpa-codex-autoban/bans

# 将单个账号加回号池
curl -X POST -H "Authorization: Bearer $CPA_MANAGEMENT_KEY" \
  -H "Content-Type: application/json" \
  -d '{"auth_id":"<AUTH_ID>"}' \
  http://localhost:8317/v0/management/plugins/cpa-codex-autoban/unban

# 清空全部插件 ban 状态
curl -X POST -H "Authorization: Bearer $CPA_MANAGEMENT_KEY" \
  http://localhost:8317/v0/management/plugins/cpa-codex-autoban/unban-all
```
...
- 日志：禁用/解禁都会通过 CPA 的日志输出（`slog`），关键字 `cpa-codex-autoban:`。
